...
## Interactive map — spatial change in sealed surfaces

Rather than summarising sealed surface change as statistics, this map shows
**where** sealing has appeared or disappeared between 2021 and 2024. Zones
in red are areas that became sealed, zones in blue are areas that lost their
sealed status. The comparison is based on a spatial intersection of the two
prediction layers, ensuring that only genuine geometric changes are shown.

::: {.callout-caution}
## Exercise 8 — Map sealed surface change (2021 → 2024) with a heatmap

**Goal:** Display the spatial distribution of sealed surface gains and losses
across the NUTS3 region using two `folium.plugins.HeatMap` layers — one for
areas that gained sealing, one for areas that lost it. Use a geometric
`overlay()` to isolate true spatial changes rather than comparing centroid
densities.

**Steps:**

1. Filter sealed polygons (label 1) from both years in EPSG:3035
2. Use `.overlay(..., how="difference")` to compute gained and lost geometries
3. Recompute `area_km2` after the overlay and filter out micro-polygons
4. Reproject to EPSG:4326, compute centroids, build `heat_gained` and
   `heat_lost` as `[lat, lon, area_km2]` lists
5. Create a `folium.Map` and add both `HeatMap` layers with distinct gradients
6. Add a `LayerControl` to toggle the two layers independently
:::
...
# Filter sealed polygons — stay in metric CRS for the overlay
sealed_2021 = gdf_nuts_2021[gdf_nuts_2021["label"] == __].to_crs("__").copy()  # TODO: label, CRS
sealed_2024 = gdf_nuts[gdf_nuts["label"] == __].to_crs("__").copy()            # TODO: label, CRS

# Geometric difference: what is in 2024 but not in 2021, and vice versa
gained = sealed_2024.overlay(__, how="__")  # TODO: reference layer, method
lost   = sealed_2021.overlay(__, how="__")  # TODO: reference layer, method

# Recompute areas after clipping and remove residual slivers
gained["area_km2"] = gained.geometry.area / 1e6
lost["area_km2"]   = lost.geometry.area   / 1e6

gained = gained[gained["area_km2"] > __]  # TODO: minimum area threshold (float)
lost   = lost[lost["area_km2"]   > __]    # TODO: minimum area threshold (float)

# Reproject to WGS84 for Folium
gained_wgs84 = gained.to_crs("__")  # TODO: CRS
lost_wgs84   = lost.to_crs("__")    # TODO: CRS

gained_wgs84["centroid"] = gained_wgs84.geometry.centroid
lost_wgs84["centroid"]   = lost_wgs84.geometry.centroid

heat_gained = [
    [row.centroid.__, row.centroid.__, row.area_km2]  # TODO: lat/lon attributes
    for _, row in gained_wgs84.iterrows()
]
heat_lost = [
    [row.centroid.__, row.centroid.__, row.area_km2]  # TODO: lat/lon attributes
    for _, row in lost_wgs84.iterrows()
]

center = gained_wgs84.geometry.centroid.union_all().centroid
m = folium.Map(location=[center.y, center.x], zoom_start=__)  # TODO: zoom level

HeatMap(
    __,           # TODO: heat_gained
    radius=__, blur=__, max_zoom=13,
    gradient={0.4: "lightyellow", 0.7: "orange", 1.0: "red"},
    name="Gained sealed 2021→2024",
).add_to(m)

HeatMap(
    __,           # TODO: heat_lost
    radius=__, blur=__, max_zoom=13,
    gradient={0.4: "lightcyan", 0.7: "dodgerblue", 1.0: "darkblue"},
    name="Lost sealed 2021→2024",
).add_to(m)
...
- Filter with `gdf_nuts_2021[gdf_nuts_2021["label"] == 1]`, same for 2024.
- Keep EPSG:3035 for the overlay so distances are in metres.
- `sealed_2024.overlay(sealed_2021, how="difference")` gives polygons in
  2024 that are not covered by 2021 — i.e. newly sealed areas.
- `sealed_2021.overlay(sealed_2024, how="difference")` gives the reverse —
  areas that were sealed in 2021 but not in 2024.
...
sealed_2021 = gdf_nuts_2021[gdf_nuts_2021["label"] == 1].to_crs("EPSG:3035").copy()
sealed_2024 = gdf_nuts[gdf_nuts["label"] == 1].to_crs("EPSG:3035").copy()

gained = sealed_2024.overlay(sealed_2021, how="difference")
lost   = sealed_2021.overlay(sealed_2024, how="difference")

gained["area_km2"] = gained.geometry.area / 1e6
lost["area_km2"]   = lost.geometry.area   / 1e6

gained = gained[gained["area_km2"] > 1e-6]
lost   = lost[lost["area_km2"]   > 1e-6]

gained_wgs84 = gained.to_crs("EPSG:4326")
lost_wgs84   = lost.to_crs("EPSG:4326")
